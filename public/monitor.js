...
function shouldAttemptReload() {
  ...
  const lastReloadAttempt = parseInt(
    localStorage.getItem("app_reload_attempt") || "0",
    10
  );
  const now = Date.now();
...
function isAppFrozen() {
  ...
  if (
    loadingElement &&
    document.body.contains(loadingElement) &&
    loadingElement.style.display !== "none" &&
    Date.now() -
      parseInt(localStorage.getItem("app_init_time") || Date.now(), 10) >
      20000
  ) {
    return true;
  }
  ...
  if (
    appRoot &&
    (!appRoot.children || appRoot.children.length === 0) &&
    Date.now() -
      parseInt(localStorage.getItem("app_init_time") || Date.now(), 10) >
      10000
  ) {
